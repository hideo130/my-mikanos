...
EFI_STATUS GetMemoryMap(struct MemoryMap *map)
{
  ...
  // gBS is global variable indecates boot service
  // memory management function is included by boot service
  // Definition of EFI_MEMORY_DESCRIPTOR is in MdePkg/Include/Uefi/UefiSpec.h of EDK2
    ...
    EFI_HANDLE image_handle, EFI_BLOCK_IO_PROTOCOL **block_io)
{
  ...
  status = gBS->OpenProtocol(
      image_handle, &gEfiLoadedImageProtocolGuid,
      (VOID **)&loaded_image,
      image_handle,
      NULL,
      EFI_OPEN_PROTOCOL_BY_HANDLE_PROTOCOL);
    ...
    EFI_SYSTEM_TABLE *system_table)
{
  ...
  // load kernel_file to temporary region
  ...
  else
  {
    ...
    if (EFI_ERROR(status))
    {
      Print(L"failed to open Block I/0 Protocol: %r\n", status);
      Halt();
    }
  ...
  // 24 is entry point addr which is spicified in elf.
