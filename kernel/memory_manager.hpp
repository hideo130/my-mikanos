...
/** @brief a size of physical memory frame.(byte)
 * This expression is user defined literal
 */
...
{
public:
    /** @brief Maximum physical memory byte this function can manage */
    static const auto kMaxPhysicalMemoryBytes{128_GiB};
    /** @brief necessary frame size which we manage range to kMaxPhysicalMemoryBytes */
    ...
    void SetMemoryRange(FrameID range_bigin, FrameID rang_end);

private:
    std::array<MapLineType, kFrameCount / kBitsPerMapLine> alloc_map_;

    FrameID range_begin_, range_end_;
